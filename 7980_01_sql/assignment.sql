...
from account where status='ACTIVE' and avail_balance > 2500;
...
from account where avail_balance <> pending_balance;
...
/* 4 */
select year(birth_date), month(birth_date), day(birth_date), weekday(birth_date)
from individual;
...
/* 8 */
select left(fname,3),right(lname,3)
from individual;

/* 9 */
select ucase(fname)
from individual where length(fname)=5;
...
/*Module 4 */
/* 1 */
select cust_id, count(*) as number_of_accounts 
from account 
group by cust_id;
...
/* 3 */
select concat(a.fname,' ',a.lname) as employee_name,
...
select fname,lname
from employee
where emp_id in(select superior_emp_id from employee where dept_id=1);
